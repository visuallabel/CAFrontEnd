...
public class PhotoSearchTask{
	...
	public PhotoList execute(){
		List<AnalysisBackend> backends = ServiceInitializer.getDAOHandler().getDAO(BackendDAO.class).getBackends(SEARCH_CAPABILITY);
		if(backends == null){
	...
	private PhotoList combineResults(UserIdentity authenticatedUser, DataGroups dataGroups, Limits limits, Collection<PhotoList> photoLists){
		...
		PhotoList results = ServiceInitializer.getDAOHandler().getDAO(PhotoDAO.class).search(authenticatedUser, dataGroups, guids, limits, null, null, null); // the back-ends are not guaranteed to return results the user has permissions to view, so re-search everything just in case
