...
public class VideoSearchTask{
	...
	private VideoList combineResults(UserIdentity authenticatedUser, DataGroups dataGroups, Limits limits, Collection<VideoList> videoLists){
		...
		VideoList results = ServiceInitializer.getDAOHandler().getDAO(VideoDAO.class).search(authenticatedUser, dataGroups, guids, limits, null, null, null); // the back-ends are not guaranteed to return results the user has permissions to view, so re-search everything just in case
