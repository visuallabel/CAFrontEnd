...
public final class VideoContentCore {
	...
	public static RedirectResponse generateTargetUrl(UserIdentity authenticatedUser, ServiceType serviceType, String guid) {		
		AccessDetails details = ServiceInitializer.getDAOHandler().getDAO(VideoDAO.class).getAccessDetails(authenticatedUser, guid);
		if(details == null){
	...
	public static VideoTaskDetails queryTaskDetails(Integer backendId, Long taskId, DataGroups dataGroups, Limits limits) {
		return ServiceInitializer.getDAOHandler().getDAO(VideoTaskDAO.class).getTask(backendId, dataGroups, limits, taskId);
	}
	...
	public static VideoList getVideos(UserIdentity authenticatedUser, List<String> guids, DataGroups dataGroups, Limits limits, EnumSet<ServiceType> serviceTypes, long[] userIdFilters) {
		return ServiceInitializer.getDAOHandler().getDAO(VideoDAO.class).search(authenticatedUser, dataGroups, guids, limits, null, serviceTypes, userIdFilters);
	}
	...
	public static Long scheduleTask(VideoTaskDetails details) {
		...
		if(taskId != null){
			...
			taskId = ServiceInitializer.getDAOHandler().getDAO(VideoTaskDAO.class).insertTask(details);
			if(taskId == null){
