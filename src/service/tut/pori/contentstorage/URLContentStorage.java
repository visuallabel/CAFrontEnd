...
public final class URLContentStorage extends ContentStorage {
	...
	public String getTargetUrl(AccessDetails details) {
		return ServiceInitializer.getDAOHandler().getDAO(URLContentDAO.class).getUrl(details.getGuid());
	}
	...
	public void removePhotoMetadata(UserIdentity userId, Collection<String> guids) {
		...
		ServiceInitializer.getDAOHandler().getDAO(URLContentDAO.class).removeEntries(remove);
	...
	public void removeVideoMetadata(UserIdentity userId, Collection<String> guids) {
		VideoDAO videoDAO = ServiceInitializer.getDAOHandler().getDAO(VideoDAO.class);
	...
	public boolean synchronizeAccount(UserIdentity userId) {
		...
		try{
			URLContentDAO urlContentDAO = ServiceInitializer.getDAOHandler().getDAO(URLContentDAO.class);
			...
			if(!entries.isEmpty()){
				...
				if(photoGUIDs.isEmpty()){
					...
					ServiceInitializer.getDAOHandler().getDAO(PhotoDAO.class).remove(photoGUIDs);
				...
				if(videoGUIDs.isEmpty()){
					...
					ServiceInitializer.getDAOHandler().getDAO(VideoDAO.class).remove(videoGUIDs);
