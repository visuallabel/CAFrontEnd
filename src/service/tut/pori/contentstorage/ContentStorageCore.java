...
public final class ContentStorageCore {
	...
	public static Long synchronize(UserIdentity authenticatedUser, int[] backendIds, EnumSet<ServiceType> serviceTypes) {
		...
		Long taskId = ServiceInitializer.getDAOHandler().getSQLDAO(TaskDAO.class).insertTask(details); // use the default implementation to insert as there is only metadata content
		if(taskId == null){
	...
	public static RedirectResponse generateTargetUrl(UserIdentity authenticatedUser, ServiceType serviceType, String guid){
		AccessDetails details = ServiceInitializer.getDAOHandler().getSolrDAO(PhotoDAO.class).getAccessDetails(authenticatedUser, guid);
		if(details == null){
	...
	public static class MetadataSynchronizationJob implements Job{
		...
		public void execute(JobExecutionContext context) throws JobExecutionException {
			...
			TaskDAO taskDAO = ServiceInitializer.getDAOHandler().getSQLDAO(TaskDAO.class);
