...
public final class FileCore {
	...
	public static File addFile(UserIdentity authenticatedUser, InputStream file, String filename) {
		...
		FileDAO fileDAO = ServiceInitializer.getDAOHandler().getSQLDAO(FileDAO.class);
	...
	public static void deleteFiles(UserIdentity authenticatedUser, long[] fileIds) {
		...
		FileList files = ServiceInitializer.getDAOHandler().getSQLDAO(FileDAO.class).delete(authenticatedUser, fileIds); // delete files, this will also check permissions
	...
	public static FileList listFiles(UserIdentity authenticatedUser, long[] fileIds, Limits limits) {
		FileList files = ServiceInitializer.getDAOHandler().getSQLDAO(FileDAO.class).getFiles(authenticatedUser, fileIds, limits);
		if(FileList.isEmpty(files)){
			return null;
		}else{
