...
public class FBSummarizationFeedbackTask extends AsyncTask {
	...
	public static void taskFinished(PhotoTaskResponse response) {
		...
		ServiceInitializer.getDAOHandler().getDAO(FBTaskDAO.class).updateTaskStatus(new BackendStatus(new AnalysisBackend(response.getBackendId()), status), taskId);
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		executeAddTask(EnumSet.of(Capability.USER_FEEDBACK, Capability.FACEBOOK_SUMMARIZATION), ServiceInitializer.getDAOHandler().getDAO(FBTaskDAO.class), getTaskId(context.getMergedJobDataMap()));
	}
}
