...
public final class FacebookExtractor {
	...
	public static FacebookExtractor getExtractor(UserIdentity userId){
		...
		OAuth2Token token = ServiceInitializer.getDAOHandler().getDAO(FacebookUserDAO.class).getToken(userId);
		if(token == null){
	...
	private Integer getWeight(WeightModifierType type){
		if(_userTagWeights == null){
			_userTagWeights = ServiceInitializer.getDAOHandler().getDAO(FacebookJazzDAO.class).getWeightModifiers(_userId);
			if(_userTagWeights == null){
		...
		if(value == null){
			if(_defaultTagWeights == null){
				_defaultTagWeights = ServiceInitializer.getDAOHandler().getDAO(FacebookJazzDAO.class).getWeightModifiers(null);
			}
			if(_defaultTagWeights == null){
	...
	public List<FacebookPhotoDescription> getPhotoDescriptions(boolean generatedTags, boolean includeEmpty){
		...
		if(retval.isEmpty()){
			...
			List<FacebookEntry> entries = ServiceInitializer.getDAOHandler().getDAO(FacebookDAO.class).getEntries(objectIds, _userId);
			if(entries == null){
