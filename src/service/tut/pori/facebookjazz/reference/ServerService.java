 ...
 * 
 * This class defines the APIs offered by the Front-end to Analysis Back-ends.
 * 
 ...
 *
 */
@HTTPService(name = service.tut.pori.facebookjazz.reference.Definitions.SERVICE_FBJ_REFERENCE_SERVER)
public class ServerService {
	 ...
	 * <h2>Example Query:</h2>
	 *
	 * POST /rest/{@value service.tut.pori.facebookjazz.reference.Definitions#SERVICE_FBJ_REFERENCE_SERVER}/{@value service.tut.pori.contentanalysis.Definitions#METHOD_TASK_FINISHED}<br>
	 * Content-Type: text/xml; charset=UTF-8<br><br>
	 *
	 * <b>[HTTP BODY STARTS]</b><br>
	 * 
	 * {@doc.restlet service="[service.tut.pori.facebookjazz.reference.Definitions#SERVICE_FBJ_REFERENCE_EXAMPLE]" method="[service.tut.pori.contentanalysis.Definitions#ELEMENT_TASK_RESULTS]" type="GET" query="" body_uri=""} <br>
	 * 
	 ...
	 * 
	 * @param xml Only the result data should be in the body. See {@link service.tut.pori.facebookjazz.FBTaskResponse}
	 */
	...
	public void taskFinished(@HTTPMethodParameter(name = InputStreamParameter.PARAMETER_DEFAULT_NAME, bodyParameter = true) InputStreamParameter xml) {
		try {
			...
			try(InputStream input = IOUtils.toInputStream(body)){ // convert back to stream for unmarshal
				FBJReferenceCore.taskFinished(_formatter.toObject(input, FBTaskResponse.class, PhotoTaskResponse.class));
			}
