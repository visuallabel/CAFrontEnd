...
public final class FacebookUserCore {
	...
	public static Response processOAuth2Callback(String authorizationCode, String errorCode, String nonce) {
		...
		Pair<String, String> nonceAndUrl = getNonceAndRedirectUri(nonce);
		...
		FacebookUserDAO dao = ServiceInitializer.getDAOHandler().getDAO(FacebookUserDAO.class);
	...
	public static Response createAuthorizationRedirection(UserIdentity authorizedUser, String redirectUri) {
		...
		uri.append(urlEncodedCombinedNonce(ServiceInitializer.getDAOHandler().getDAO(FacebookUserDAO.class).generateNonce(authorizedUser), redirectUri));
	...
	public static Response removeAuthorization(UserIdentity authenticatedUser) {
		ServiceInitializer.getDAOHandler().getDAO(FacebookUserDAO.class).removeToken(authenticatedUser); // simply remove the token, as it is not possible to revoke facebook tokens, they should auto-expire after a certain time
	 ...
	 */
	private static String urlEncodedCombinedNonce(String nonce, String redirectUri){
		String retval = null;
		try {
			if(StringUtils.isEmpty(redirectUri)){
	 ...
	 */
	private static Pair<String, String> getNonceAndRedirectUri(String nonce){
	...
	private static class UserEventListener implements ApplicationListener<UserServiceEvent>{
		...
		public void onApplicationEvent(UserServiceEvent event) {
			...
			if(type == EventType.USER_REMOVED || (type == EventType.USER_AUTHORIZATION_REVOKED && event.getSource().equals(UserCore.class) && UserServiceType.FACEBOOK.equals(event.getUserServiceType()))){
				...
				ServiceInitializer.getDAOHandler().getDAO(FacebookUserDAO.class).removeToken(userId);
